...
This document provides a global summary of the 13 specifications in [openspec/specs](file:///root/proj/go/agentic-layer-custom/openspec/specs). It maps the custom agentic gateway architecture using the 4+1 View Model.
...
*   GLM-5 Provider: Implements provider wrappers for OpenAI-compatible chat endpoints targeting DashScope GLM-5 when LLM_PROVIDER is set to glm5. It replaces the previous Kimi provider implementation.
    *   Spec References: [glm5-provider spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/glm5-provider/spec.md) and [kimi-provider spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/kimi-provider/spec.md)
    *   Code Implementation: [glm.go](file:///root/proj/go/agentic-layer-custom/pkg/model/glm.go) and [openai_compatible.go](file:///root/proj/go/agentic-layer-custom/pkg/model/openai_compatible.go)
*   Base URL Normalization: Automatically formats configured endpoints to accommodate both full completions paths and basic API hosts.
    *   Spec Reference: [glm5-provider spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/glm5-provider/spec.md)
...
*   System Agent (Router): Determines user intents and routes them to appropriate downstream worker agents (such as the Connection Agent). It asks for manual clarification when prompts are highly ambiguous.
    *   Spec Reference: [system-agent spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/system-agent/spec.md)
    *   Code Implementation: [system.go](file:///root/proj/go/agentic-layer-custom/pkg/agents/system.go)
*   Connection Agent: Runs a ReAct (Reason + Act) loop using adk-go. It loads custom skill definitions on startup and delegates state management, parameter passing, and tool orchestration to the LLM.
    *   Spec Reference: [connection-agent spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/connection-agent/spec.md)
    *   Code Implementation: [connection.go](file:///root/proj/go/agentic-layer-custom/pkg/agents/connection.go)
...
*   Universal Mock Tool: Implements a generic Go tool registered under custom names (e.g., Auth_tool) that logs execution to stdout and returns mock maps with common fields (like status and token).
    *   Spec Reference: [universal-mocking spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/universal-mocking/spec.md)
    *   Code Implementation: [signaling.go](file:///root/proj/go/agentic-layer-custom/pkg/tools/signaling.go)
...
*   Intents API (/v1/intents/stream): Establishes a client connection to stream reasoning events, tool execution status, and final summaries in response to user prompts.
    *   Spec Reference: [websocket-api spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/websocket-api/spec.md)
    *   Code Implementation: [websocket.go](file:///root/proj/go/agentic-layer-custom/pkg/api/websocket.go)
*   Skill Generation API (/ws/agent-run): Executes the multi-agent skill writer pipeline, sending progress updates and validation diagnostics to the client.
    *   Spec Reference: [skill-generation spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/skill-generation/spec.md)
    *   Code Implementation: [ws.go](file:///root/proj/go/agentic-layer-custom/pkg/workshop/ws.go)
...
*   Granular Telemetry Streamer: Captures prompts, LLM thought chunks (specifically recognizing thought structures with Thought: true), and network events, publishing them as structured JSON objects (ai_payload, llm_thought, network_pcap, workflow_complete).
    *   Spec References: [telemetry-streamer spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/telemetry-streamer/spec.md) and [agent-tracing spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/agent-tracing/spec.md)
    *   Code Implementation: [telemetry.go](file:///root/proj/go/agentic-layer-custom/pkg/telemetry/telemetry.go)
...
*   Format Check & Iterative Correction: Uses the format checker agent to validate generated markdown against the tool catalog schema (retrieved via GET `/api/tools`), performing up to 3 repair attempts on validation failures before rejecting the run.
    *   Spec Reference: [skill-generation spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/skill-generation/spec.md)
    *   Code Implementation: [orchestrator.go](file:///root/proj/go/agentic-layer-custom/pkg/workshop/orchestrator.go)
...
*   WebSocket API Tests: Confirms endpoint messaging formats and correct response sequences.
    *   Spec Reference: [websocket-api spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/websocket-api/spec.md)
    *   Tests: [websocket_test.go](file:///root/proj/go/agentic-layer-custom/pkg/api/websocket_test.go)
*   Dynamic Orchestration Tests: Verifies that Connection Agent dynamically registers skills by parsing CALL patterns inside SKILL.md templates.
    *   Spec Reference: [skill-orchestration spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/skill-orchestration/spec.md)
    *   Tests: [skill_orchestration_test.go](file:///root/proj/go/agentic-layer-custom/pkg/agents/skill_orchestration_test.go)
*   Workshop Orchestrator Tests: Verifies format correction repair limits (max 3 loops) and output generation.
    *   Spec Reference: [skill-generation spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/skill-generation/spec.md)
    *   Tests: [orchestrator_test.go](file:///root/proj/go/agentic-layer-custom/pkg/workshop/orchestrator_test.go)
...
*   dotenv Support: Gateway loads local configuration parameters from a `.env` file at startup.
    *   Spec Reference: [env-config spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/env-config/spec.md)
    *   Code Implementation: [main.go](file:///root/proj/go/agentic-layer-custom/cmd/agent-gateway/main.go) using joho/godotenv.
...
*   Web Dashboard Integration: Launches a local server (defaulting to http://localhost:8080) hosting a browser-based UI. The interface allows operators to select active agents (SystemAgent, ConnectionAgent) and visualize tracing logs.
    *   Spec Reference: [web-dashboard spec](file:///root/proj/go/agentic-layer-custom/openspec/specs/web-dashboard/spec.md)
...
3.  The Connection Agent loads skill configurations from [skill/init-registration/SKILL.md](file:///root/proj/go/agentic-layer-custom/skill) and identifies unique tool requirements (e.g. `CALL "Auth_tool"` and `CALL "Subscription_tool"`).
...
6.  The final valid `SKILL.md` is registered and written into the [skill](file:///root/proj/go/agentic-layer-custom/skill) registry directory.
